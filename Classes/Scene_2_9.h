...
class Scene_2_9 : public Scene_UI
{
public:
	...
	virtual void removeRes()
	{
		...
		SpriteFrameCache::getInstance()->removeSpriteFramesFromFile("Scene_2/Object01-hd.plist");
		SpriteFrameCache::getInstance()->removeSpriteFramesFromFile("Scene_2/Monsters01-hd.plist");
	...
	virtual void initScene(Node*p)
	{
		...
		if (rand() % 2 == 0)
		{
			SimpleAudioEngine::getInstance()->playBackgroundMusic("Music/BGMusic02.mp3", true);
		}
		else
		{
			SimpleAudioEngine::getInstance()->playBackgroundMusic("Music/BGMusic12.mp3", true);
		}
		base_money = 750;
		key_to_ = "2_9";

		auto bg = Sprite::create("Scene_2/BG0/BG1-hd.pvr.png");
		p->addChild(bg);
		bg->setPosition(WIDTH / 2, 640 / 2);
		

		auto path__ = Sprite::create("Scene_2/BG9/BG-hd.pvr.png");
		path__->setPosition(WIDTH / 2,0);
		p->addChild(path__);
		path__->setAnchorPoint(Vec2(0.5,0));

		SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Scene_2/Object01-hd.plist");
		SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Scene_2/Monsters01-hd.plist");

		current_level = 9;
		callback_next = [=]()
		{

			Director::getInstance()->replaceScene(Scene_2_10::createScene());
		...
		callback_restart = [=]()
		{
			Director::getInstance()->replaceScene(Scene_2_9::createScene());
		};
