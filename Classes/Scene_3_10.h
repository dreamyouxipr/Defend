...
class Scene_3_10: public Scene_UI
{
public:
	...
	virtual void removeRes()
	{
		...
		SpriteFrameCache::getInstance()->removeSpriteFramesFromFile("Scene_3/Object02-hd.plist");
		SpriteFrameCache::getInstance()->removeSpriteFramesFromFile("Scene_3/Monsters01-hd.plist");

	}

	virtual void initScene(Node*p)
	{
		SimpleAudioEngine::getInstance()->playBackgroundMusic("Music/BGMusic03.mp3", true);

		base_money = 550;
		key_to_ = "3_10";

		auto bg = Sprite::create("Scene_3/BG0/BG2-hd.pvr.ccz");
		p->addChild(bg);
		bg->setPosition(WIDTH / 2, 640 / 2);
		

		auto path__ = Sprite::create("Scene_3/BG10/BG-hd.pvr.ccz");
		path__->setPosition(WIDTH / 2,0);
		p->addChild(path__);
		path__->setAnchorPoint(Vec2(0.5,0));
		SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Scene_3/Monsters01-hd.plist");

		SpriteFrameCache::getInstance()->addSpriteFramesWithFile("Scene_3/Object02-hd.plist");
		current_level = 10;
		callback_next = [=]()
		{

			Director::getInstance()->replaceScene(Scene_3_11::createScene());


		};
		callback_restart = [=]()
		{
			Director::getInstance()->replaceScene(Scene_3_10::createScene());
		};
