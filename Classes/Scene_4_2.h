...
class Scene_4_2: public Scene_UI
{
public:
	...
	virtual void initScene(Node*p)
	{
		...
		auto bg = Sprite::create("Scene_4/BG0/BG1-hd.pvr.ccz");
		p->addChild(bg);
		bg->setPosition(WIDTH / 2, 640 / 2);


		auto path__ = Sprite::create("Scene_4/BG2/BG-hd.pvr.ccz");
		path__->setPosition(WIDTH / 2+30, 0-5);
		p->addChild(path__);
		...
		callback_next = [=]()
		{
			...
			Director::getInstance()->replaceScene(Scene_4_3::createScene());
	...
	callback_restart = [=]()
	{
		Director::getInstance()->replaceScene(Scene_4_2::createScene());
	};
